...
export interface StreamBufferCallbacks {
  ...
  /**
   * Fired when a text segment is sealed (fully received from SSE).
   * Used by TTS to queue audio generation for the completed text.
   */
  onSegmentSealed?: (
    messageId: string,
    partId: string,
  ...
  /**
   * When provided and returns true, the buffer will hold on the current text
   * item after it's fully revealed. Used to wait for TTS audio to finish.
   */
  ...
  private sealLastText(): void {
    for (let i = this.items.length - 1; i >= 0; i--) {
      ...
      if (item.kind === 'text' && !item.sealed) {
        item.sealed = true;
        // Notify TTS system that this segment is ready
