...
export function buildCourseContext(ctx?: SceneGenerationContext): string {
  ...
  if (ctx.speechHistory && ctx.speechHistory.length > 0) {
    ...
    console.log("Using fallback speech context for this slide");
    ...
    console.log("No course speech context available for this slide");
  }
...
export function formatImageDescription(img: PdfImage, language: string): string {
  ...
  if (img.width && img.height) {
    ...
    dimInfo = ` | Size: ${img.width}×${img.height} (${ratio} aspect ratio)`;
  }
...
export function buildVisionUserContent(
  ...
  if (images.length > 0) {
    ...
    for (const img of images) {
      ...
      if (img.width && img.height) {
        const ratio = (img.width / img.height).toFixed(2);
        dimInfo = ` (${img.width}×${img.height}, ${ratio} aspect ratio)`;
      }
      parts.push({ type: 'text', text: `\n**${img.id}**${dimInfo}:` });
